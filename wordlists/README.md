# offsec wordlist

**Curated collection for penetration testers and red teamers**  
*Custom-crafted wordlists marked with 501 prefix*

## 🌞 Custom-Crafted Wordlists (501 Series)

### `501-api-wild.txt`
```text
Description: Contains api paths seen in the 'wild', so stuff that I've come across during my testing.
Purpose: API endpoint discovery
Sources: 
- My own research material in the wild
```

### `501-big-clean-list-of-naughty-attack-strings.txt`
```text
Description: Fusion of SecLists's **big-list-of-naughty-strings.txt** and wfuzz's **All_attacks.txt**. Cleaned up and ready to roll.
Purpose: Large-scale input validation testing
Lines: 935
Sources:
- big-list-of-naughty-strings.txt (SecLists)
- All_attacks.txt (wfuzz)
```

## 📦 Standard Wordlists

###  `api_docs_path`
```text
Description: Wordlist crafted for fuzzing api documentation. Quite comprehensive.
Purpose: Discover documentation.
Lines: 236
```